...
// Keep WebRTC from exposing a direct UDP/local-network candidate outside the
// configured route. This must be set before Electron becomes ready.
...
ipcMain.handle('open-secure-browser', async (_event, value, options={}) => {
	...
	const webrtcMode=options.webrtcMode === 'proxy_only' ? 'proxy_only' : 'blocked';
	const allowsMedia=permission=>webrtcMode === 'proxy_only' && ['media','display-capture'].includes(permission);
	browserSession.setPermissionRequestHandler((_webContents,permission,callback)=>callback(allowsMedia(permission)));
	browserSession.setPermissionCheckHandler((_webContents,permission)=>allowsMedia(permission));
